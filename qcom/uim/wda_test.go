...
func TestWDARequestEncoding(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantMessage: qcom.MessageWDAGetDataFormat,
		},
		{
		...
		},
		{
			...
			wantMessage: qcom.MessageWDASetDataFormat,
			wantTLVs: map[byte][]byte{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.req.Service != qcom.ServiceWDA {
				t.Fatalf("Service = 0x%02X, want 0x%02X", tt.req.Service, qcom.ServiceWDA)
			}
...
func TestWDADataFormatResponseUnmarshalTLVs(t *testing.T) {
	tests := []struct {
		name    string
		tlvs    tlv.TLVs
		check   func(*testing.T, qcom.WDADataFormat)
		wantErr bool
	}{
		{
			...
			tlvs: tlv.TLVs{
				...
				tlv.Uint(wdaTLVLinkProtocol, uint32(qcom.WDALinkLayerRawIP)),
				tlv.Uint(wdaTLVUplinkAggregation, uint32(qcom.WDAAggregationQMAP)),
				tlv.Uint(wdaTLVDownlinkAggregation, uint32(qcom.WDAAggregationQMAP)),
				...
				tlv.Uint(wdaTLVResponseQoSHeader, uint32(qcom.WDAQoSHeader6Bytes)),
				tlv.Uint(wdaTLVResponseDownlinkPadding, uint32(4)),
				tlv.Uint(wdaTLVResponseFlowControl, uint8(1)),
			},
			check: func(t *testing.T, got qcom.WDADataFormat) {
				if !got.LinkLayerProtocolKnown || got.LinkLayerProtocol != qcom.WDALinkLayerRawIP {
					t.Fatalf("LinkLayerProtocol = %d, known %v", got.LinkLayerProtocol, got.LinkLayerProtocolKnown)
				}
				if !got.UplinkAggregationKnown || got.UplinkAggregation != qcom.WDAAggregationQMAP {
...
func TestSetWDALinkLayerProtocolUsesWDA(t *testing.T) {
	tests := []struct {
		name     string
		protocol qcom.WDALinkLayerProtocol
	}{
		{name: "raw IP", protocol: qcom.WDALinkLayerRawIP},
		{name: "Ethernet", protocol: qcom.WDALinkLayerEthernet},
	}
				...
				calls: []transportCall{
					{
						check: func(req qcom.Request) {
							assertTLV(t, req.TLVs, 0x01, []byte{byte(qcom.ServiceWDA)})
						},
						resp: successResponse(qcom.MessageAllocateClientID, tlv.Bytes(0x01, []byte{byte(qcom.ServiceWDA), 5})),
					},
					{
						check: func(req qcom.Request) {
							if req.Service != qcom.ServiceWDA || req.ClientID != 5 || req.MessageID != qcom.MessageWDASetDataFormat {
						...
						resp: successResponse(qcom.MessageWDASetDataFormat, tlv.Uint(wdaTLVLinkProtocol, uint32(tt.protocol))),
					},
					{
						check: func(req qcom.Request) {
							assertTLV(t, req.TLVs, 0x01, []byte{byte(qcom.ServiceWDA), 5})
						},
