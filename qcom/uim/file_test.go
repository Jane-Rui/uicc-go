			...
			transport := &fakeTransport{
				...
				calls: []transportCall{{
					check: func(req qcom.Request) {
						if req.Service != qcom.ServiceUIM || req.MessageID != qcom.MessageWriteRecord {
					...
					resp: successResponse(qcom.MessageWriteRecord, tlv.Bytes(qmiTLVCardResult, []byte{0x90, 0x00})),
				}},
			}
...
func TestWriteRecordValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			reader := &Reader{transport: &fakeTransport{t: t}, slot: 1, clientID: 7}
		...
		t.Run(tt.name, func(t *testing.T) {
			reader := &Reader{
				transport: &fakeTransport{t: t, calls: []transportCall{{
					resp: successResponse(qcom.MessageWriteRecord, tt.tlvs...),
				}}},
				slot:     1,
				clientID: 7,
			}
