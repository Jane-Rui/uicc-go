...
func resultOK(resp qualcomm.Response) error {
	return qualcomm.ResultError(resp.TLVs)
}

func cardResultOK(resp qualcomm.Response) error {
...
func decodeServiceVersions(resp qualcomm.Response) ([]serviceVersion, error) {
	...
	for i := range count {
		...
		versions = append(versions, serviceVersion{
			Service: qualcomm.ServiceType(value[offset]),
