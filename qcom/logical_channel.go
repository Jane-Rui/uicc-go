...
const (
	...
	maxSendAPDUCommandLength = MaxQMUXServiceTLVLength - sendAPDUFixedTLVLength
)
...
func (c *Client) OpenLogicalChannel(ctx context.Context, aid []byte) (uint8, error) {
	...
	resp, err := c.request(ctx, MessageOpenLogicalChannel, tlv.TLVs{
...
func (c *Client) SendAPDU(ctx context.Context, channel uint8, command []byte) ([]byte, error) {
	...
	resp, err := c.request(ctx, MessageSendAPDU, tlv.TLVs{
