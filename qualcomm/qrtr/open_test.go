...
func (d *fakeDialer) Dial(_ context.Context, service qualcomm.ServiceType) (packetConn, error) {
	d.service = service
	return d.conn, d.err
}
...
func TestOpenUsesDialer(t *testing.T) {
	tests := []struct {
		...
		{"custom service", qualcomm.ServiceCAT, &fakeDialer{conn: fakeConn{}}, false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if wantService == 0 {
				wantService = qualcomm.ServiceUIM
			}
