...
func (c *Conn) findService(serviceType qualcomm.ServiceType) (*service, error) {
	...
	for time.Now().Before(timeout) {
		...
		if qualcomm.ServiceType(service.Service) == serviceType {
			return &service, nil
		}
	}
...
func (c *Conn) sendControlPacket(serviceType qualcomm.ServiceType) error {
	pkt := &controlPacket{
